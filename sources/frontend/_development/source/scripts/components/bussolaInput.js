        ...
        function switchModalOff(e, self) {
            if (
                $(e.target)
                    .parent()
                    .parent()
                    .hasClass("bussola_hideonloading_input")
            ) {
        ...
        if (true) {
            ...
            this.awesomplete = new Awesomplete(_inputComplete, {
                ...
                filter: (data, input) => {
                    ...
                    if (input.length <= 2 && _formatRegexp(input).toLowerCase() == _formatRegexp(data.value.split(" - ")[1]).toLowerCase() && estados.includes(input.toUpperCase())) {
                        // if (_formatRegexp(input).toLowerCase() == _formatRegexp(data.value.split(" - ")[1]).toLowerCase()) {
                        return true;
                        // }
                    } else if (input.length <= 2 && _formatRegexp(input).toLowerCase() != _formatRegexp(data.value.split(" - ")[1]).toLowerCase() && !estados.includes(input.toUpperCase())) {
                        return data.value.toLowerCase().indexOf(input.toLowerCase()) != -1;
                        //     return data.value.toLowerCase().indexOf(input.toLowerCase()) != -1;
                    } else if (input.length > 2) {
                        //     console.log("caiua qui?");
                        return data.value.toLowerCase().indexOf(input.toLowerCase()) != -1;
                    }
                }
            });
