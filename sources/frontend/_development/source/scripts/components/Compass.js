...
class Compass {
    ...
    setTemplateBussola(origin, estado, cidade, ddd) {
        ...
        if (origin == "cookie-success" || origin == "geoip-success" || origin == "geolocation-success") {
            ...
            $(this.bussolaMainSelector + " #submit")
                .css("display", "block")
                .attr({
                    "data-estado": estado,
                    "data-cidade": cidade,
    ...
    showCompass(event, locatedByGeoIp, estado, cidade, ddd, origem, type_compass, exibiuBussola, selecionouCidade, escapouBussola) {
        $(this.bussolaMainSelector).fadeIn(100);
        // this.datalayer.sendDataBussola("show-compass", "exibiu-bussola", estado, cidade, ddd, "", type_compass);
    }
    ...
    configAutocomplete() {
        ...
        if (!window.bussolaIsInit) {
            this.awesomplete = new Awesomplete(_inputComplete, {
                ...
                filter: (data, input) => {
                    ...
                    if (!this.searchByState) {
                        // If is first run of the filter
                        this.filteredByStates = [];
                        this.cities.cidades.map(cidade => {
                            // Create an array with all cities of founded state
                            this.searchByState = true;
                            if (_formatRegexp(input).toLowerCase() == _formatRegexp(cidade.value.split(" - ")[1]).toLowerCase()) {
                                // Ex: - SP | - GO
                    ...
                    if (this.filteredByStates.indexOf(data.label) > -1) {
                        // If found by state:
                        return true;
                    }
                    if (!this.unfilteredByStates) {
                        // If not found by state:
                        return (
                            _formatRegexp(data)
                                .toLowerCase()
                                .indexOf(_formatRegexp(input).toLowerCase()) > -1
                        );
                    }
                }
            });
            ...
            $(_inputComplete).on("input", () => {
                ...
                if (_inputCompleteLength >= 2) {
                    ...
                    if (!suggestions || suggestions.length < 1) {
                        ...
                        this.showAwesompletePreUl();
                        $(this.awesomplete.ul).html($(Awesomplete.ITEM('Ops! Não encontramos essa cidade. <br class="only-mobile"/> Tente digitar de novo.', "")).addClass("bussola_noresults"));
                    }
            ...
            $(this.awesomplete.ul)
                .parent()
                .append('<div class="' + this._awesompletePreUl.replace(".", "") + '" hidden=""></div>');
    ...
    createDefaultRecomendations() {
        ...
        for (var city in this.citiesPriorityUnreversed) {
            if (i < (this.helpers.isMobile() ? 5 : 4)) {
                $(this.awesomplete.ul).removeAttr("hidden");
                $(this.awesomplete.ul).append(
                    $(Awesomplete.ITEM(this.citiesPriorityUnreversed[city], "")).on("click", e => {
                        // this.cityChosedByRecomendation = true;
                        // this.setCurrentCity($(e.target).text());
                        console.log("1");
                        this.hideBussolaList();
